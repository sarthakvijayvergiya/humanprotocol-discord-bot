...
class DatabaseManager:
    ...
    async def get_user_settings(self, user_id: int):
        """
        This function retrieves the result channel ID for a user.

        :param user_id: The ID of the user.
        :return: A tuple containing result channel ID.
        """
        async with self.connection.execute(
            "SELECT result_channel_id FROM user_settings WHERE user_id = ?",
            (user_id,)
        ) as cursor:
            return await cursor.fetchone()
